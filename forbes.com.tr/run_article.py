"""
Single command: pass URL → fetch HTML (first article.col.col7 + div.col.col10); with flags run parser and/or AI JSON.
...
class ForbesRunner(BaseRunner):
    def __init__(self):
        super().__init__(site_dir=Path(__file__).resolve().parent)

    def get_description(self) -> str:
        return "Fetch Forbes Türkiye article by URL; with --parse run parser to JSON, with --ai generate JSON via Anthropic API."

    def get_url_help(self) -> str:
        return "Full article URL (e.g. https://www.forbes.com.tr/ekonomi/...)"
