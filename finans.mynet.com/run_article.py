"""
Single command: pass URL → fetch HTML (only detail-content-box); with flags run parser and/or AI JSON.
...
class FinansMynetRunner(BaseRunner):
    def __init__(self):
        super().__init__(site_dir=Path(__file__).resolve().parent)

    def get_description(self) -> str:
        return "Fetch Mynet Finans article by URL; with --parse run parser to JSON, with --ai generate JSON via Anthropic API."

    def get_url_help(self) -> str:
        return "Full article URL (e.g. https://finans.mynet.com/haber/detay/...)"


if __name__ == "__main__":
    FinansMynetRunner().main()
