"""
Parse HTML from HTML_files/ (page without my-header, sc-heading-container, affiliate-sidebar) into scraped_article_json_schema format, write to Parsed_files/.
"""
...
def _get_metadata(soup: BeautifulSoup, base_url: str) -> dict:
    ...
    if author_el:
        ...
        if name:
            name = re.sub(r"\s+MYNET YAZARI\s*$", "", name, flags=re.I).strip()
        if name:
...
def _components_from_detail_content(container: Tag) -> list:
    ...
    for child in container.children:
        if not isinstance(child, Tag):
            continue
        if _has_class(child, "interest-widget-container", "content-banner-box", "detail-footer-tags"):
            continue
        ...
        if child.name == "hr":
            continue
        if child.name == "a" and _has_class(child, "affiliate-label", "affiliate-article-btn"):
            continue
    return components
...
def parse_article_html(html_raw: bytes, base_url: str = BASE_URL) -> dict:
    ...
    feature_box = soup.find("div", class_=lambda c: c and "showcase-feature-box" in (c if isinstance(c, str) else " ".join(c)))
    if feature_box:
        feature_media = feature_box.find("div", class_=lambda c: c and "feature-media" in (c if isinstance(c, str) else " ".join(c)))
        if feature_media:
            img = feature_media.find("img", class_=lambda c: c and "img-responsive" in (c if isinstance(c, str) else " ".join(c)))
            if img:
                url = (img.get("data-original") or img.get("src") or "").strip()
                if url:
                    props = {"url": url}
    ...
    if not components_list or components_list[0].get("type") != "image":
        feature_media = soup.find("div", class_=lambda c: c and "feature-media" in (c if isinstance(c, str) else " ".join(c)))
        if feature_media:
            img = feature_media.find("img", class_=lambda c: c and "img-responsive" in (c if isinstance(c, str) else " ".join(c)))
            if not img:
            ...
            if img:
                ...
                if url:
                    ...
                    if alt:
                        props["description"] = alt
                    lead_img = {"type": "image", "properties": props}
                    if components_list:
                        components_list.insert(0, lead_img)
                    else:
                        components_list.append(lead_img)

    content_box = soup.find("div", class_=lambda c: c and "showcase-detail-content-box" in (c if isinstance(c, str) else " ".join(c)))
    if not content_box:
