...
class TurkiyegazetesiRunner(BaseRunner):
    def __init__(self):
        super().__init__(site_dir=Path(__file__).resolve().parent)

    def get_description(self) -> str:
        return "Fetch article HTML by URL; with --parse run parser to JSON, with --ai generate JSON via Anthropic API."

    def get_url_help(self) -> str:
        return "Full article URL (e.g. https://www.turkiyegazetesi.com.tr/ekonomi/...)"


if __name__ == "__main__":
    TurkiyegazetesiRunner().main()
